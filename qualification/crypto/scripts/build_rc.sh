#!/usr/bin/env bash
# crypto / publish-candidates: build reprodutível da wheel/sdist rc do cripto-predictor.
...
/c/QUALIFICACAO/tools/python/cpython-3.13.14-windows-x86_64-none/python.exe -m zipfile -l "$OUT"/dist1/*.whl | grep -E "GarimpoInvestimentos/(output/reporter|research_runner|research_faults|adapters/__init__|charters/scientific_state)"
