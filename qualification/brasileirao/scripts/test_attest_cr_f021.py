...
`environments`, nos veredictos compartilhados (`verdict_sha256`) e no `findings_file`; e só aceita
núcleo antigo (v2.0/v2.1) em parcial IN_PROGRESS já emitido, nunca numa attestation final.
...
def test_fresh_partial_passes(doc: dict) -> None:
    ...
    assert doc["common_core_version"] == "2.2" and doc["common_core_sha256"] == attest.CORE_SHA
...
def test_old_core_only_for_in_progress(doc: dict) -> None:
    old = copy.deepcopy(doc)
    old["common_core_sha256"] = attest.CORE_SHA_HIST[0]
