"""stocks / fase soak: executa QUALIFICATION_PROFILE_STOCKS_V1 pelo entrypoint instalado.
...
--tests), cada chamada ao `stocks-research` num processo novo. Um JSON por linha no log
...
def main() -> int:
    ...
    for i in range(20):
        ...
        if i % 4 == 0 and i // 4 < 5:  # 5 duplicatas
            dup = request(f"stocks:REQ-SOAK-N-{i:03d}", client_ref={"dup": i})
        ...
        if i % 4 == 1 and i // 4 < 5:  # 5 restarts em pontos de morte do processo, em rodízio
            ...
            rrid = f"stocks:REQ-SOAK-R-{i:03d}"
    ...
    for k in range(3):
        for case, dataset, want in (("A", "case_a", ("INCONCLUSIVE", "NO_EDGE")), ("B", "case_b", ("SUPPORTED", "NO_EDGE"))):
            rid = f"stocks:REQ-SOAK-CASE{case}-{k}"
    ...
    # perfil stocks: 3 família-não-pronta (0 trial elegível) e 5 coletas válidas em COLLECTION_ONLY
    from conformance.fixtures import collection_request

    not_ready = []
    for k in range(3):
        rid = f"stocks:REQ-SOAK-EI-{k}"
        value = request(rid)
    ...
    collections = []
    for k in range(5):
        rid = f"stocks:REQ-SOAK-COLLECT-{k}"
        value = collection_request(rid, as_of=f"2026-09-2{k}T00:00:00Z",
                                   parameters={"collector": "cvm-vlmo", "period": "2026",
                                               "observed_at": f"2026-09-1{k + 5}T12:00:00Z"})
        code, outcome = run(env, f"col{k}", value)
    ...
    for rid in expected_results:
        ...
        if result is not None and (result.get("capital_permission") is not False
                                   or not all(str(result.get(f, "")).startswith("stocks:")
    ...
    for rid in expected_results:
        ...
        if row is None:
            continue
        stored_result = json.loads(bytes(row[0]))
        eligible_trials += int(bool(stored_result.get("trial_eligible")))
        for reb in (stored_result.get("domain_facts") or {}).get("rebalances") or []:
    ...
    ok = (not lost and not extra and not reread_mismatch and not violations and code == 0
          and eligible_trials == 0 and not pit_violations
